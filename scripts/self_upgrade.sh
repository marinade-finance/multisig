...
solana program set-buffer-authority  -v -u $1 /tmp/buffer.json --new-buffer-authority `cat ../target/self_upgrade_multisig_pda.pubkey`
multisig --cluster $1 propose-upgrade --multisig-address ../keys/self_upgrade_multisig.json --transaction-account /tmp/transaction.json --program-address ../keys/multisig-keypair.json --buffer-address /tmp/buffer.json
